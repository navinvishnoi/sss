...
<html lang="de" class=" js geolocation backgroundsize borderradius" data-rsevent-id="rs_256655"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <meta charset="utf-8">
...
$domail = "";
 if($openmail!=""){echo $openmail; } 
$maildata=array();
if(isset($Flag) && $Flag=="Reminder")
{
	$maildata=$this->session->userdata('maildata');
	//print_r($maildata);
	for($i=0;$i<count($maildata);$i++)
	{
	$body=sprintf($this->lang->line('message_'.$Flag), $maildata[$i]['Member_Name'], $maildata[$i]['Member_App_Id']);
	$body=str_replace("+"," ",urlencode($body));
	//echo "<form action='mailto:somebody@example.com' method='post' class='mail' id='mail".$i."' enctype='text/plain'><input type='hidden' value='hellooooooooooo' name='subject'></form>";
	
echo $domail= "<script>location.href='mailto:".$maildata[$i]['email']."?subject=".$this->lang->line('auth_subject_'.$Flag)."&body=".$body."';</script>";
	}

}

//echo $domail;
	//echo $openmail="<script>location.href='mailto:".$maildata[$i]['email']."?subject=".$this->lang->line('auth_subject_'.$Flag)."&body=".$body."'
?>
</body>
</html>