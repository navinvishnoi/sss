...
class Admin extends CI_Controller {
	...
	$data['userdata']=$this->admin_model->GetBufferData(session_id());
	if(count($data['userdata'])==0){
	$this->admin_model->InsertBufferData(array('SessionId'=>session_id()));
	$data['userdata']=$this->admin_model->GetBufferData(session_id());
	}
	...
	/*if($this->session->userdata('Flag')!="")
		...
		if($this->session->userdata('Flag')=="Accepted")
			...
			}elseif($this->session->userdata('Flag')=="Rejected")
			{
				//$data['Flag'] = $this->session->userdata('Flag');
			...
			elseif($this->session->userdata('Flag')=="Check_App_ID")
			{
				$data['getMessagebyTitle']=$this->admin_model->getMessagebyTitle("Check_App_ID");
			}
			elseif($this->session->userdata('Flag')=="Reminder")
			{
				//$data['Flag'] = $this->session->userdata('Flag');
