...
class Admin extends CI_Controller {
	...
	public function SearchFilterSessioln()
	{
		...
		return $this->admin_model->AdminSearch($filterdata ,$filterdatadate);
		}
	public function index()
	{	$this->session->set_userdata('Flag',"Index");
	$data=$this->getdatafromdb();
	echo $this->FLAG;
	echo $this->session->userdata('Flag');
	//$this->session->set_flashdata('Flag',"Accepted");
		/*echo $this->session->userdata('Flag');
		if($this->session->userdata("Flag")!="")
		{echo $this->session->userdata('Flag');die();
			//$this->session->keep_flashdata('Flag');
		/*$this->session->keep_flashdata('Member_App_Id');
		$this->session->keep_flashdata('Email');
		$this->session->keep_flashdata('Link');
			if($this->session->userdata('Flag')=="Accepted")
			{
			...
			}elseif($this->session->flashdata('Flag')=="Rejected")
			{
				...
				$data['getmemberbyid']=$this->admin_model->getmemberbyid($memberid);
				$this->admin_model->updateMemberStatus($data['Member_App_Id']);
				$data['Email']=$this->session->flashdata('Email');
			...
			elseif($this->session->userdata('Flag')=="Update")
			{
				$data['Flag'] = $this->session->userdata('Flag');
				$Member_App_Id=$this->session->flashdata('Member_App_Id');
				$memberid=$this->CheckUniqueAppID($Member_App_Id);
		...
		if($this->session->userdata('FilterValue')!="All")
		{
			$data['getmemberlist']=$this->SearchFilterSessioln();
		}else
		{
...
function compare()
{
	...
	if($memberappid==""){//$this->session->set_flashdata('Flag',"Check_App_ID");
	redirect('admin/');}
...
<a href='".base_url()."admin/exportdata' title='Daten exportieren' >Daten exportieren</a>
	...
	function Accept()
	{
		...
		{
		$countryvalue='';$filterdatadate=array();
		...
		$Member_Current_Status="Accepted";
		...
		$data['Member_App_Id'] = $memberAppid;
		$data['Email'] = $email;
		$data['Flag'] = $type;
		$memberid=$this->CheckUniqueAppID($memberAppid);
		$data['getmemberbyid']=$this->admin_model->getmemberbyid($memberid);
		$data['getMessagebyTitle']=$this->admin_model->getMessagebyTitle($type);
		$data['getmemberlist']= $this->admin_model->AdminSearch($type,$filterdatadate);
		//$this->load->view('admin/index', $data);
		$this->FLAG="Accepted";
		redirect('admin/');
		}
	...
	function updateMail()
	{
			$type="Update";$maildata;$filterdatadate=array();
			$memberappid=$this->uri->segment(3);
