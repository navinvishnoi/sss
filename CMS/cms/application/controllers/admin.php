...
class Admin extends CI_Controller {
	...
	*/ 
	...
	{ $sessionid = $_GET['sessionid'];$this->session_exists($sessionid); }
	...
	foreach($data['userdata'] as $dataval)
	{
		...
		if($dataval->Flag==="Delete")
		{$data['getmemberbyid']=$this->admin_model->getanymemberbyappid($dataval->memberAppId);}
		else
		...
		if($dataval->FilterFlag!="")
		{
			$filterdata='';$filterdatadate=array();
			$FilterValue = $dataval->FilterFlag;
			...
			elseif($FilterValue=='Rejected'){$filterdata='Rejected';
			//$data['AdminSearch']=$this->admin_model->AdminSearch($filterdata ,$filterdatadate);
			}
			elseif($FilterValue=='Accepted'){$filterdata='Accepted';
			//$data['AdminSearch']=$this->admin_model->AdminSearch($filterdata ,$filterdatadate);
			}
			...
			elseif($FilterValue=="Request_For_Deletion"){
				$filterdata='Request_For_Deletion';
				}//get the member list according to session filter value.
			$data['FilterFlag'] = $dataval->FilterFlag;
			$data['getmemberlist']=$this->admin_model->AdminSearch($filterdata ,$filterdatadate);
		}else
		{
			$userdata=array('FilterFlag'=>"All");
			$this->admin_model->UpdateBufferData($userdata,$sessionid);//sets the session data for filter value
			$data['FilterFlag'] = "All";
...
public function search()
{
		...
		elseif($filterval[0]=='Accepted'){$filterdata='Accepted';
			//$data['AdminSearch']=$this->admin_model->AdminSearch($filterdata ,$filterdatadate);
			}	
		...
		elseif($filterval[0]=='six'){$btntype="submit";
			...
			elseif($filterval[0]=="Request_For_Deletion"){$filterdata='Request_For_Deletion';}
			//get search data into data array ...			
			$data['AdminSearch']=$this->admin_model->AdminSearch($filterdata ,$filterdatadate);
			$userdata=array('FilterFlag'=>$filterval[0]);
			$this->admin_model->UpdateBufferData($userdata,$sessionid);//sets the session data for filter value
...
foreach($data['AdminSearch'] as $value)
	{  
		 ...
		 $show.= "<tr><td>".$status."</td>
					...
					<td class='center'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='".base_url().'admin/updateMail/'.$value->Member_App_Id."/".$sessionid."' title='Aktualisierung anfordern'>
					<img src='".asset_url()."img/orange_icons/mail_24.png' class='btn13' title='".$this->lang->line("Mailto")."'></a>
				  </td></tr>";
			$Member_CreatedAt1='';$Member_latest_Release1='';}
			$show.="</table>";
	}
	else
	{$show="<div class='col-lg-12 pull-left' style='margin-left:-15px;'>".$this->lang->line("No_records")."</div>";}
...
function reject()
	{
		...
		{//$memberappid=$memberval->Member_App_Id;
			if($memberval->Member_Current_Status==="New" || $memberval->Member_Current_Status==="New_Rejected" 
				|| $memberval->Member_Current_Status==="New_Request_For_Deletion" )
			{	$type="New_Rejected";
				$link="member_author/profile/"; 
				$MemberCurrentStatus="New_Rejected";//Rejected
			}
			...
			{
				$link="member_author/profile/";
				$MemberCurrentStatus="Rejected_After_Accepted";
				//$this->admin_model->updateMemberlogStatus($memberlogid);that deactive the updated log profile. 
			}
	...
	function delete()
	{
			...
			foreach($data['getmemberbyid'] as $memberval)
			{$email=$memberval->MA_email;}
			$this->admin_model->updateMemberCurrentStatus($memberappid,$type);
